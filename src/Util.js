...
class Util {
  constructor() {
    this.db = low(adapter);
    this.usage = {
      rename: 'Usa: .rename <Antigo> <Novo>',
      remove: 'Usa: .remove <Audio>',
      ignore: 'Usa: .ignore <usuario>',
      unignore: 'Usa: .unignore'
    };
  }
  ...
  getListOfCommands() {
    return [
      ...
      '',
      'commands              Mostra essa msg, acho que vc sabe',
      'sounds                Mostra todos os sons',
      'mostplayed            Mostra os 15 mais usados',
      'sound                 roda um som ',
      'random                Toca um som aleatorio',
      'stop                  Para os sons e limpa a fila',
      'rename <Antigo> <Novo>Renomeia um som ',
      'ignore <usuario>      Ignore specified user',
  ...
  Volume(input, message){
    if(input>=0 && input<=4){
      return input;
    }else{
      message.reply('O Volume deve ser Entre 0.0 e 4 seu idiota');
    }
  }
}
